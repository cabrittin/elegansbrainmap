...
def plot_clustermap(fin,cfg,bundles,no_ticklabels=False,no_cbar=False,reorder=False,fig_title=None):
    ...
    #bundles = ioaux.read.into_dict(cfg['clusters']['noise'])
    bcolor = dict([(d[2],d[1]) for d in ioaux.read.into_list2(cfg['mat']['bundle_color'])])
    border = ioaux.read.into_list(cfg['mat']['bundle_order'])
...
def plot_compare(cfg,im):
    ...
    L4 = ioaux.read.into_dict('data/clusters/clusters_s23_JSH_t35.csv')
    M4 = ioaux.read.into_dict('data/clusters/clusters_s23_m4_t35.csv')
    Adult = ioaux.read.into_dict('data/clusters/clusters_s23_N2U_t35.csv')
    
    #L4 = ioaux.read.into_dict('data/clusters/clusters_s23_JSH_t35_opt.csv')
    #M4 = ioaux.read.into_dict('data/clusters/clusters_s23_m4_t35_opt.csv')
    #Adult = ioaux.read.into_dict('data/clusters/clusters_s23_N2U_t35_opt.csv')
    ...
    ioaux.write.from_list('mat/cluster_node_order.txt',nodes)
    ...
    #ioaux.write.from_list('results/cluster_revision/fig1_row_order.csv',data)
    #ioaux.write.from_list('results/cluster_revision/edfig5i_row_order.csv',data)
...
def _plot_compare(cfg,im):
    ...
    cnum = 8
    M4 = ioaux.read.into_dict(cin%cnum) 
    #M4 = ioaux.read.into_dict('data/clusters/clusters_s23_m4_t35.csv')
    L4 = ioaux.read.into_dict('data/clusters/clusters_s23_JSH_t35.csv')
    Adult = ioaux.read.into_dict('data/clusters/clusters_s23_N2U_t35.csv')
